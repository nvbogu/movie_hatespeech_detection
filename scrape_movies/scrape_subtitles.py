...
def download_zip(url,movie_n,folder):
	...
	movie_n_new = movie_n.replace("?", "")
	movie_n_new = movie_n.replace(".", "")
	with open(os.path.join('movie_subtitles_zips/', folder+movie_n_new+'.zip'), 'wb') as temp_file:
...
for file_name in glob.iglob('movie_names/movie_names_racism.txt', recursive=True):
	movie_list = []
	with open(file_name,'r') as f:
		movie_list.extend(f.read().split('\n'))
	print("FILENAME:  ", file_name)
	if 'friendship' in file_name:
		folder = 'friendship/'
		...
		if containers:
			...
			for row in rows:
			    ...
			    if cols_parsed[1]=='English':
			    	...
			    	print(download_url)
			    	download_zip(download_url, movie, folder)
			    	break
